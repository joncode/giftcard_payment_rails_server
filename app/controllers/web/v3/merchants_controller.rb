class Web::V3::MerchantsController < MetalCorsController
	...
	def show
		merchant = Merchant.unscoped.find_with(params[:id])
	...
	def redemption_locations
		# merchant ID , affiliate ID , menu item ID
		# item_type, item_id
		item_type = params[:item_type].underscore
		item_id = params[:item_id].to_i

		case item_type
		when "merchant"
			item = Merchant.unscoped.find_with(item_id)
		when "affiliate"
			item = Affiliate.unscoped.find(item_id)
		when "menu_item"
		...
		serialized = item.multi_redemption_merchants.map(&:web_serialize)
		success(serialized)
		respond
	end
	...
	def redeem_locations
		merchant = Merchant.unscoped.find_with(params[:id])
		if client = merchant.multi_redemption_client
...
private
	...
	def merchant_signup_params
		...
		params.require(:data).permit(:address, :venue_name, :venue_url,
			 :point_of_sale_system, :name, :email, :phone, :position, :message, :price, :ccy )
