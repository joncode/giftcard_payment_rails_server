...
private
	...
	def self.check_for_previous_campaign_gift args
		...
		if user = self.find_user(args)
			u_gifts = Gift.where(giver_id: campaign.id, giver_type: "Campaign", receiver_id: user.id).where('created_at > ?', Time.now - 8.hours).count
		end
		n_gifts = if args["rec_net"] == 'em'
			Gift.where(giver_id: campaign.id, giver_type: "Campaign", receiver_email: args["rec_net_id"]).where('created_at > ?', Time.now - 8.hours).count
