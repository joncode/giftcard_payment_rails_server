...
# "token"=>"LlWODlRC9M3VDbzPHuWMdA"}
	...
	def self.get_history_provider_and_range(provider, start_date=nil, end_date=nil)
		if start_date && end_date
			Gift.where(provider_id: provider.id, status: 'redeemed').where("updated_at >= :start_date AND updated_at <= :end_date", :start_date => start_date.in_time_zone, :end_date => end_date.in_time_zone ).order("updated_at DESC")
		else
