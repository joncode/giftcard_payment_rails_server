module QaTeam
    ...
    def self.erase
        ...
        puts "my administrator account (support@itson.me) which was never used for testing, and Deb Fennell's account (qa@itson.me).  All of my other accounts which were deleted (cmcaulay71@yahoo.com and cmmca71@gmail.com) were used for testing, so I can re-create those accounts during my next QA session."
        users = User.all.to_a
        deleted_users = []
        # loop thru each user
        users.each do |user|
