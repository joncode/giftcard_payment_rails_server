...
describe Mdot::V2::UsersController do
    ...
    describe :deactivate_user_social do
        ...
        it_should_behave_like("token authenticated", :put, :deactivate_user_social, id: 1)

        it "should return user ID on success" do
            request.env["HTTP_TKN"] = "USER_TOKEN"
            put :deactivate_user_social, format: :json, id: @user.id, identifier: @user.email, type: "email"
            rrc(200)
            json["status"].should == 1
            json["data"].should   == @user.id
            put :deactivate_user_social, format: :json, id: @user.id, identifier: @user.phone, type: "phone"
            rrc(200)
            json["status"].should == 1
            json["data"].should   == @user.id
            put :deactivate_user_social, format: :json, id: @user.id, identifier: @user.facebook_id, type: "facebook_id"
            rrc(200)
            json["status"].should == 1
            json["data"].should   == @user.id
            put :deactivate_user_social, format: :json, id: @user.id, identifier: @user.twitter, type: "twitter"
            rrc(200)
            json["status"].should == 1
            json["data"].should   == @user.id
        end
        ...
        it "should return 404 with no ID or wrong ID" do
            ...
            put :deactivate_user_social, format: :json, id: @user.id, identifier: user2.email, type: "email"
            rrc(404)
            put :deactivate_user_social, format: :json, id: @user.id, identifier: user2.phone, type: "phone"
            rrc(404)
        end
    end
