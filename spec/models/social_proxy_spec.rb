...
describe SocialProxy do
    ...
    context "friends" do
        ...
        context "twitter" do
            ...
            it "should reply 401 - cannot reach server when auth fails" do
                stub_request(:post, route).with(:body => "data[token]=#{@oauth_hsh_tw["token"]}&data[secret]=#{@oauth_hsh_tw["secret"]}&data[network_id]=#{@oauth_hsh_tw["network_id"]}&data[handle]=#{@oauth_hsh_tw["handle"]}", :headers => {'Accept'=>'application/json', 'Authorization'=>"#{SOCIAL_PROXY_TOKEN}"}).to_return(:status => 401, :body => "", :headers => {})
                require_hsh  = @oauth_hsh_tw
                social_proxy = SocialProxy.new(require_hsh)
                social_proxy.friends
                social_proxy.status.should == 401
