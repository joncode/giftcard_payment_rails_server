...
describe Omnivore do
	...
	describe "gift pos_redeem" do
		...
		it "should NOT redeem BRAND CARD when ticket has menu item id incorrect" do
			p = FactoryGirl.create(:provider, :pos_merchant_id => "EaTaa5c6", :r_sys => 3)
			u = FactoryGirl.create(:user)
			g = make_gift_sale(u, u, "100", p.id)
			g.update(shoppingCart:  "[{\"detail\":\"red curry, coconut cream, lime\",\"price\":\"13\",\"photo\":\"d|v1428517736/ae3vsptiglipdl3qbbgl.jpg\",\"price_promo\":\"\",\"pos_item_id\":\"500-5006\",\"item_id\":3388,\"item_name\":\"Thai Chicken Flatbread\"}]")
			g.update(brand_card: true)
			if g.status != 'open'
				g.update_column(:status, 'open')
			end
			g.notify
			...
			resp["response_code"].should  == "ERROR"
			resp["response_text"].should  == "This gift card is only good for the specific item.  We did not find the specific on the ticket."
			g.reload.status.should == 'notified'

		end

		it "should redeem BRAND CARD when ticket has menu item id correct" do
			p = FactoryGirl.create(:provider, :pos_merchant_id => "EaTaa5c6", :r_sys => 3)
			u = FactoryGirl.create(:user)
			g = make_gift_sale(u, u, "100", p.id)
			g.update(shoppingCart:  "[{\"detail\":\"red curry, coconut cream, lime\",\"price\":\"13\",\"photo\":\"d|v1428517736/ae3vsptiglipdl3qbbgl.jpg\",\"price_promo\":\"\",\"pos_item_id\":\"recb5cKX\",\"item_id\":3388,\"item_name\":\"Thai Chicken Flatbread\"}]")
			g.update(brand_card: true)
			if g.status != 'open'
				g.update_column(:status, 'open')
			end
